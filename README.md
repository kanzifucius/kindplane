...
- **Raw GitHub (fallback)**: `https://raw.githubusercontent.com/kanzifucius/kindplane/main/kindplane.schema.json`

**Automatic Schema Detection**

Files generated by `kindplane init` include this comment at the top:

```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/kanzifucius/kindplane/main/kindplane.schema.json
```

For versioned schemas, use the GitHub Pages URL:
...
For the best experience, install the [YAML extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml). The schema reference comment enables:
...
If you have an existing configuration file without the schema comment, add it to the first line:
...
Alternatively, configure schema association in your VS Code/Cursor settings:
...
```bash
kindplane validate                # Validate kindplane.yaml
kindplane validate --config my.yaml  # Validate a specific file
```
